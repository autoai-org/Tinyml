...
def build_reversed_model(x, model):
    ...
    for layer in model.features:
        if isinstance(layer, nn.Conv2d):
            ...
            deconv_layer = nn.ConvTranspose2d(layer.out_channels, layer.in_channels, layer.kernel_size, layer.stride, layer.padding)
        ...
        if isinstance(layer, nn.MaxPool2d):
            ...
            unpool_layer = nn.MaxUnpool2d(kernel_size=layer.kernel_size, stride=layer.stride, padding=layer.padding)
            unpool_layer.return_indices = False
            layer_lists.append(unpool_layer)
    layer_lists.reverse()
    return nn.Sequential(*layer_lists), pooling_indices

def backward_featuremaps(y, model, layer_count, pooling_indices, num_of_layers):
    output = y
    ...
    for i, layer in enumerate(model):
        if (i >= (num_of_layers - layer_count - 1)):
            ...
            if isinstance(layer, nn.MaxUnpool2d):
                ...
                output = layer(output[0], pooling_indices[num_of_layers-layer_count-1])
            else:
                output = layer(output)
    return output
...
if __name__ == '__main__':
    ...
    for i, layer in enumerate(model.features):
        ...
        reconstructed_img = backward_featuremaps(raw_features, demodel, i, pooling_indices, num_of_layers)
