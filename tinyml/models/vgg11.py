...
def vgg11():
    ...
    # Conv Block 1
    model.add(Conv2D('b_1_conv2d_1', (3, 224, 224), n_filter=8,
                    w_filter=3, h_filter=3, stride=1, padding=1))
    model.add(ReLu('b_1_relu_1'))
    model.add(Conv2D('b_1_conv2d_2', (8, 224, 224), n_filter=8,
                    w_filter=3, h_filter=3, stride=1, padding=1))
    model.add(ReLu('b_1_relu_2'))
    model.add(MaxPool2D('b_1_pool_1', (8, 224, 224),
                        (2, 2), stride=2, return_index=True))

    # Conv Block 2
    model.add(Conv2D('b_2_conv2d_1', (8, 112, 112), n_filter=16,
                    w_filter=3, h_filter=3, stride=1, padding=1))
    model.add(ReLu('b_2_relu_1'))
    model.add(Conv2D('b_2_conv2d_2', (16, 112, 112), 16, 3, 3, 1, 1))
    model.add(ReLu('b_2_relu_2'))
    model.add(MaxPool2D('b_2_pool_1', (16, 112, 112), (2, 2), 2, return_index=True))
    ...
    model.add(MaxPool2D('b_3_pool_1', (32, 56, 56), (2, 2), 2, return_index=True))
    ...
    model.add(MaxPool2D('b_4_pool_1', (64, 28, 28), (2, 2), 2, return_index=True))
