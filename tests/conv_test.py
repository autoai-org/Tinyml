...
class TestConv2D(unittest.TestCase):
    ...
    def test_convolution(self):
        ...
        self.assertTrue((torch_conv_output.detach().numpy() - tnn_conv_output <
                         EPSILON).all())
...
class TestConv2D_multiple_channel(unittest.TestCase):
    ...
    def test_backward(self):
        ...
        self.assertTrue(
            (np.absolute(self.torch_conv.weight.grad.numpy() -
                         self.tnn_conv.weight.gradient < EPSILON)).all())
        self.assertTrue(
            (np.absolute(self.torch_conv.bias.grad.numpy() -
                         self.tnn_conv.bias.gradient.T) < EPSILON).all())
        self.assertTrue(
            (np.absolute(self.torch_input.grad - out_grad) < EPSILON).all())
